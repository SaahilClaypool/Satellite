...
def parse_file(filename="./data/ping_trace.txt"):
    ...
    df['systemTime'] = df['systemTime'].apply(
        lambda x: pd.Timestamp(x / 1000, unit='s'))
    return df
...
def windowed_loss(df, window='60s'):
    ...
    for ind, ri in r:
        ...
        loss_windows = loss_windows.append(
            [{'systemTime': ind, 'loss': loss}]).dropna()
...
def plot_sampled(df, output_dir='./graphs/rolling', title="quantile", quantiles=[0.0, .25, .5, .75, 1.0], names=["min", "lower", "median", "upper", "max"], window='60s'):
    ...
    for name, frame in frames:
        plt.scatter(frame.index - timedelta(hours=4),
                    frame['rtt'], alpha=0.50, s=5, label=name)
    ...
    plt.plot(loss_windows.index - timedelta(hours=4),
             loss_windows['loss'], label='loss (%)')
    plt.xlabel("time ")
    plt.ylabel("loss percent")
    plt.ylim(bottom=0)
    plt.title(title + ' loss')
...
def plot_ccdf(df, output_dir="./graphs/cdf", title="ccdf"):
    ...
    sorted_rtts = df['rtt'].sort_values().reset_index()
    plt.scatter(sorted_rtts.rtt, (1 - sorted_rtts.index /
                                  sorted_rtts.rtt.count()) * 100)
    plt.ylabel("Percent")
